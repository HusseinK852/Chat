...
>
          ...
          <v-divider />
          <v-list>
            <v-list-item class="conversations">
...
const postGeminiKey = async () => {
  try {
    const response = await axios.post('http://localhost:8000/api/v1/gemini/get-gemini-token', { geminiApiKey: geminiApiKeyInput.value })
